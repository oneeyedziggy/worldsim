...
function guid(){
  ...
  function makeGuid(){
    return btoa(Date.now()).substr(0,18) + '-' +
           s4() + s4() + '-' + s4() + s4();
  };
  return makeGuid();
};
...
function agent(object){
	...
	if(logging){console.log(this.agentID);}
	//})();
	...
	agent.prototype.setResourceValue = function(resourceName, attribName, value){
		this.maxRecName = "";
		this.minRecName = "";
	...
	agent.prototype.getPlan = function(numberOfSteps){
		...
		function planStep(agent,parentStep,fd,ed,md,depth,maxDepth,pathSoFar){
			...
			if(depth == maxDepth ){
				...
				this.valAvg = (this.fVal+this.eVal+this.mVal)/3.00;
				...
				this.aVal = (this.valAvg + (fBuff+eBuff+mBuff))-(fNerf+eNerf+mNerf);

				//console.log('this');
				//console.log(this);
				if(this.aVal > planEndNode.aVal && depth != 0){
					planEndNode = this;
				}else if(this.aVal == planEndNode.aVal && depth != 0 && randrange(0,1) == 1 ){
				...
				var energyPerFood = (energyPrice+0.00)/(foodPrice+0.00);
				//planStep(agent,parentStep,fd,ed,md,depth,maxDepth,pathSoFar)
				var stepF = new planStep(agent,this,1,0,(foodPrice*-1),depth+1,maxDepth,pathSoFar+"F");
				var stepE = new planStep(agent,this,-1,energyPerFood,0,depth+1,maxDepth,pathSoFar+"E");
				var stepM = new planStep(agent,this,0,-1,energyPrice,depth+1,maxDepth,pathSoFar+"M");
		...
		var planTree = planStep(this,{"fVal":parentFood,"eVal":parentEnergy,"mVal":parentMoney},0,0,0,0,numberOfSteps,"");
	...
	agent.prototype.getMaxValRecName = function(){ 
		...
		if(this.maxRecName == ""){ //effectively cache until changed
			//for (resourceIndex in this.resources){
			for (var i = 0; i < this.resourcesIndex.length; i++){
				var resourceIndex = this.resourcesIndex[i];
				var resource = this.resources[resourceIndex];
...
resource = function(){
	...
	//function __init__(this){
		...
		this.preference = (randrange(10,90)+0.00)/100.00;
...
for(i=0;i<100;i++){
	...
	if(i == 0){
		...
		console.log('watchAgent = '+ watchAgent);
	}
	agentIDs.push(newAgent.agentID);
	//agents.push(newAgent);
	agents[newAgent.agentID] = newAgent;
}
console.log('agent length:' + associativeArrayLen(agents));
...
var watchF = agents[agentIDs[x]].getResourceValue('food','quantity');
var watchE = agents[agentIDs[x]].getResourceValue('energy','quantity');;
var watchM = agents[agentIDs[x]].getResourceValue('money','quantity');;
console.log('watchF: '+watchF+', watchE: '+watchE+', watchM: '+watchM);
...
function updateCharts(){
	...
	if(document.getElementById('metricForm').metric.value == "price"){
		var chartValueArrays = regionResourcePrices.slice();
	}else if(document.getElementById('metricForm').metric.value == "quantity"){
		var chartValueArrays = regionResourceQuantities.slice();
	}else if(document.getElementById('metricForm').metric.value == "supply"){
		var chartValueArrays = regionResourceSupplies.slice();
	}else if(document.getElementById('metricForm').metric.value == "demand"){
	...
	var ctxRegion0 = document.getElementById("region0Chart").getContext("2d");
	var chtR0Chart = new Chart(ctxRegion0).Line(dataR0, dataOpts);
	var ctxRegion1 = document.getElementById("region1Chart").getContext("2d");
	var chtR1Chart = new Chart(ctxRegion1).Line(dataR1, dataOpts);
	var ctxRegion2 = document.getElementById("region2Chart").getContext("2d");
	var chtR2Chart = new Chart(ctxRegion2).Line(dataR2, dataOpts);
	var ctxRegion3 = document.getElementById("region3Chart").getContext("2d");
	var chtR3Chart = new Chart(ctxRegion3).Line(dataR3, dataOpts);

	var ctxFood = document.getElementById("foodChart").getContext("2d");
	var chtFood = new Chart(ctxFood).Line(dataF, dataOpts);
	var ctxEnergy = document.getElementById("energyChart").getContext("2d");
	var chtEnergy = new Chart(ctxEnergy).Line(dataE, dataOpts);
...
function doIteration(){
	...
	for(var i = 0; i < regions.length; i++){
		...
		for(var j = 0; j < potentialResources.length; j++){
			...
			var currRegionResource = regions[i].resources[regions[i].resourcesIndex[j]];
	...
	for(currAgentIdIndex in agentIDs){
		...
		if(currAgent.state == 1){
			...
			if(logging){console.log('tryToGet result:' + result.toString());};
	...
	if(output.indexOf(watchAgent) > -1){
		console.log('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!');
		console.log('watchAgent is now');
		console.log('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!');
	}
	//TradesMadePerIteration.push(output);//

	console.log(agents[watchAgent]);
	console.log('watchAgent food: '+ agents[watchAgent].resources['food'].quantity.toString() + ', energy: ' + agents[watchAgent].resources['energy'].quantity.toString() + ', money: ' + agents[watchAgent].resources['money'].quantity.toString());
	if(logging){
		console.log('maxResourceCounts: food='+maxResourceCounts['food'].toString()+', money='+maxResourceCounts['money'].toString()+', energy='+maxResourceCounts['energy'].toString());
		console.log('agents:');console.log(agents);
		...
		console.log(watchAgent + " minRec = " + agents[watchAgent].getMinValRecName())
		console.log(watchAgent + " maxRec = " + agents[watchAgent].getMaxValRecName())
		for (rec in potentialResources){
			console.log(potentialResources[rec] + ' val for ' + watchAgent.toString() + ' = ' + agents[watchAgent].getResourceValue(potentialResources[rec], 'quantity'))
		}
		for(kk in agentIDs){
	...
	console.log('agentsProcessedPerIteration:');
	console.log(agentsProcessedPerIteration);
	console.log('TradesMadePerIteration:');
	console.log(TradesMadePerIteration);
	console.log('regionResourceQuantities:');
	console.log(regionResourceQuantities);
	console.log('regionResourceSupplies:');
	console.log(regionResourceSupplies);
	console.log('regionResourceDemands:');
	console.log(regionResourceDemands);
	for(var i = 0; i < regions.length; i++){
		console.log('region '+i.toString()+': ');
		for(var j = 0; j < potentialResources.length; j++){
			console.log(potentialResources[j].toString()+' prices: ' );
			console.log(regionResourcePrices[i][j]);
			console.log(regionResourcePrices[i][j].length.toString());
		}
		j = 0;
		console.log('');
	}
...
function tryToGet(boughtResource, buyerId){
	...
	if(logging){console.log('trying to get ' + boughtResource + ' for ' + buyerId);}
	...
	if (typeof isAvail != 'string' && sellerId && (sellerId != buyerId || boughtResource == "energy" )){					
		if(logging){console.log('and there\'s some available from ' + sellerId.toString() + ' in exchange for some ' + sellerWantedRec.toString());}
		...
		var paymentQuantity = getExchangeRate(buyerId, boughtResource); 
		//console.log('');
...
function getExchangeRate(agent, resourceType){
	...
	var foodPerEnergy = (moneyPerFood+0.00)/((1/energyPerMoney)+0.00);
...
function transfer(resource, from, to, quantity){
	if(logging && (from  == watchAgent || to == watchAgent)){
		console.log('transfering ' + quantity.toString() + ' ' + resource.toString() + ' from ' + from.toString() + ' to ' + to.toString());
	}
...
function resolveNecessity(minRecName, asking){
	...
	if(minRecName == "food"){ 
		...
		if(agents[asking].getResourceValue('money','quantity') > 2){
			//convert money to food
			return "food";
		}else{// if(i have no money){
			if(agent[asking].getResourceValue('energy','quantity') > 2){
				//try to work for money (to buy food)
				return "money";
			}else{// if(i have no energy){
				fucked(asking);
				return "fucked";
			}
		}
	}else if(minRecName == "energy"){
		if(agents[asking].getResourceValue('food','quantity') > 2){
			//convert some food to energy
			return "energy";
		}else{// if(i have no food){
			if(agent[asking].getResourceValue('money','quantity') > 2){
		...
		if(agents[asking].getResourceValue('energy','quantity') > 2){
			//try to work for money
			return "money";
		}else{// if(i have no energy){
			if(agent[asking].getResourceValue('food','quantity') > 2){
				//convert some food to energy
				return "energy";
			}else{// if(i have no food){
				fucked(asking);
...
function isAvailableB(recName, asking){ 
	...
	if(logging){console.log("checking availability");}
	...
	for(var i = 0; i < innerAgentIDs.length; i++){
		...
		if(asking != innerAgentID &&
			currSellers.indexOf(innerAgentID) < 0){
			...
			// true || 
			if( (askerMin == "food" && iaMin == "money") ||  //askerMax == "money" is implied later by askerMax == iaMin as is iaMax == "food"
			   (askerMin == "money" && iaMin == "energy")){ //askerMax == "energy" is implied later by askerMax == iaMin as is iaMax == "money"
				needsAlign = true;
			}else{
				needsAlign = false;
			...
			if(askerHasMin && offererHasMax && needsAlign){
				if(askerMinLessThaniaMax && iaMinLessThanAskerMax){
					if(logging || innerAgentID == watchAgent || asking  == watchAgent){
						console.log('askerID = ' + asking.toString() + ' offererID = ' + innerAgentID);
					}
...
function rollover(input, max){ 
	var  output = '';
	if(logging){
		console.log('rolling ' + input.toString() + 'over a max of ' + max.toString());
	};
...
function fucked(fuckee){
	if(logging){console.log(fuckee.toString()+', you\'re fucked');}
...
function getStats(){
	...
	for (rec in potentialResources){
		...
		for(x in agentIDs){
			var currRecVal = agents[agentIDs[x]].getResourceValue(potentialResources[rec], 'quantity');
			
			recStats["listOf"+potentialResources[rec]].push(currRecVal);
			recStats["total"+potentialResources[rec]] += currRecVal;
		}
		recStats["avg"+potentialResources[rec]] = recStats["total"+potentialResources[rec]]/associativeArrayLen(agents);
		recStats["med"+potentialResources[rec]] = median(recStats["listOf"+potentialResources[rec]]);
	}
...
function testCountRegions(){
	...
	for(var currAgentIdIndex in agentIDs){
		...
		for(var i = 0; i < numberOfRegions; i++){
			...
			if((currAgent.position.x >= regions[i].topLeft.x && 
...
function testSupDem(){
	...
	for(var i = 0; i < numberOfRegions; i++){
		...
		for(var j = 0; j < currRegion.agentsIndex.length; j++){
			...
			for(var k = 0; k < potentialResources.length; k++){
				var currResourceName = potentialResources[k];
				var currAgentCurrRecVal = currAgent.getResourceValue(currResourceName, 'quantity');
...
function testRegionalCalcPrices(){
	...
	for(var i = 0; i < numberOfRegions; i++){
		...
		for(var k = 0; k < potentialResources.length; k++){
			var currResourceName = potentialResources[k];
			if(potentialResources[k] != 'money'){
				nonMoneyValueMultiplier += (quantityOfNonMoney/currRegion.resources[currResourceName].quantity);
			}
		}
		...
		for(var k = 0; k < potentialResources.length; k++){
			...
			if(potentialResources[k] != 'money'){
				...
				//this is for debug only... add "*(demand/supply)" back in later
				currResource.price = ((quantityOfMoney/nonMoneyValueMultiplier)*(quantityOfNonMoney/currResourceQuantity))/currResourceQuantity;
